...
const bookingCount = async (req, res) => {
  try {
      const { date, time, transportType } = req.query;
      ...
      const schedulesSnapshot = await schedulesRef.where('time', '==', time).where('transportType','==',transportType).get();

      if (schedulesSnapshot.empty) {
          return res.json({ count: 0 }); // No schedules found for the time
      }
